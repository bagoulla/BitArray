...
TEST(SpeedTest, speedtest) {
  ...
  auto load_duration_baseline =
      duration_cast<milliseconds>(load_stop_baseline - load_start_baseline);
  ...
  std::cout << "Loading a bool vector took: " << load_duration_baseline.count()
            << "ms" << std::endl;
  std::cout << "Loading a BitArray took: " << load_duration.count() << "ms"
            << std::endl;
  ...
  auto dotprod_duration_baseline = duration_cast<milliseconds>(
      dotprod_stop_baseline - dotprod_start_baseline);

  auto dotprod_start = high_resolution_clock::now();
  uint64_t my_value =
      BitArray::DotProd(testArray1[start_a], testArray2[start_b], num);
  auto dotprod_stop = high_resolution_clock::now();
  auto dotprod_duration =
      duration_cast<milliseconds>(dotprod_stop - dotprod_start);

  std::cout << "DotProd a bool vector took: "
            << dotprod_duration_baseline.count() << "ms" << std::endl;
  std::cout << "DotProd a BitArray took: " << dotprod_duration.count() << "ms"
            << std::endl;
